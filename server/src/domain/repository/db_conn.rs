...
pub trait DBConn: Send + Sync {
    async fn query(&mut self, query: &str) -> Result<Vec<MySqlRow>, Error>;
    async fn query_one(&mut self, query: &str) -> Result<MySqlRow, Error>;
    async fn execute(&mut self, query: &str) -> Result<u32, Error>;

	async fn query_with_params(&mut self, query: &str, params: &[&(dyn sqlx::Encode<MySql> + std::marker::Sync)]) -> Result<Vec<MySqlRow>, Error>;
	async fn query_one_with_params(&mut self, query: &str, params: &[&(dyn sqlx::Encode<MySql> + std::marker::Sync)]) -> Result<MySqlRow, Error>;
    async fn execute_with_params(&mut self, query: &str, params: &[&(dyn sqlx::Encode<MySql> + std::marker::Sync)]) -> Result<u32, Error>;
}
